...
public class SingleShotGun : Gun
{
	...
	void Update()
	{
		...
		if (Input.GetKeyUp(KeyCode.Mouse0) && isReloading == false )
		{
			gunAnimator.enabled = false;

        }
		Debug.Log(playerController.itemIndex);
	}
	void Shoot()
	{
        ...
        if (currentAmmo > 0 )
		{
            ...
            if (canShoot == true)
			{
                ...
                isShooting = true;
                if (playerController.itemIndex == 0 && Time.timeSinceLevelLoad >= nextTimeToFire)
                {
